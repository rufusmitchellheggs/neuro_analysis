...
#--------------------------------------------------------------------------------------------------------------
    ...
    if np.array(trace_trimmed['Time (s)'])[-1]-np.array(dlc['Time (s)'])[-1] > 1:
        gpio_end = gpio_start+np.array(dlc['Time (s)'])[-1]
...
#--------------------------------------------------------------------------------------------------------------
    ...
    for i in range(len(trace_trimmed)):
        if i < door_frame_vid:
            ...
            position.append(12)
        elif distance.euclidean(sandwells[0], [dlc.transpose()[1][i], dlc.transpose()[2][i]]) < sw_radius:
            position.append(1)
        elif distance.euclidean(sandwells[1], [dlc.transpose()[1][i], dlc.transpose()[2][i]]) < sw_radius:
            position.append(2)
        elif distance.euclidean(sandwells[2], [dlc.transpose()[1][i], dlc.transpose()[2][i]]) < sw_radius:
            position.append(3)
        else:
            position.append(0)
